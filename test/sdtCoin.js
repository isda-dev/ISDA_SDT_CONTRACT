...
/**
 * Sdt contract learning test
 *   deploy in before, and no deploy between it()
 */
contract("SdtCoin", (accounts) => {
  ...
  let NoOfTokensInWei, OneSdtInWei, NoOfTokensInSDT;
  let coin
  ...
  before(async () => {
    ...
    NoOfTokensInWei = BIG(await coin.totalSupply());
    OneSdtInWei = BIG(await coin.getOneSdt());
    NoOfTokensInSDT = NoOfTokensInWei.div(OneSdtInWei)
  });
  ...
  it("coin initial values", async () => {
    ...
    assert.equal(await coin.symbol(), "SDT");
    assert.equal(await coin.name(), "ISDA Sports Data Token");
    assert.equal(await coin.decimals(), 18);
    assert.equal((await coin.totalSupply()).toString(), BIG(NOOFTOKENS).mul(OneSdtInWei).toString());
    assert.equal(await balanceOf(owner), "0");  // accounts[1]0
    assert.equal(await balanceOf(admin), "0");  // 0
    assert.equal(await balanceOf(vault), NoOfTokensInWei);
  ...
  it("token distribution", async () => {
    ...
    await coin.transfer(admin, OneSdtInWei, {from: vault});
    assert.equal(await balanceOf(admin), OneSdtInWei.toString());

    // transfer 0.5 token from admin to owner
    let halfPlayX = OneSdtInWei.div(BIG(2))
    await coin.transfer(owner, halfPlayX, {from: admin});
    assert.equal(await balanceOf(owner), halfPlayX.toString());
    assert.equal(await balanceOf(admin), halfPlayX.toString());
    assert.equal(await balanceOf(vault), (NoOfTokensInSDT.sub(BIG(1)).mul(OneSdtInWei).toString()));
  });
  ...
  it("marginal transfers", async () => {
    ...
    try {
      await coin.transfer(user2, OneSdtInWei.mul(BIG(1)), {from: user1});
      assert.fail();
    }
    ...
    try
    {
      await coin.transfer(user2, OneSdtInWei.mul(BIG(0)), {from: user1});
      assert.equal(await balanceOf(user1), OneSdtInWei.mul(BIG(0)).toString());
      assert.equal(await balanceOf(user2), OneSdtInWei.mul(BIG(0)).toString());
    }
    catch(exception) { assert.fail(); }

    // transfer 0 token from vault(balance > 0) to user1
    try {
        await coin.transfer(user1, OneSdtInWei.mul(BIG(0)), {from: vault});
        assert.equal(await balanceOf(user1), OneSdtInWei.mul(BIG(0)).toString());
    } catch(exception) { assert.fail(); }

    // transfer whole balance to other
    await coin.transfer(user1, OneSdtInWei.mul(BIG(1)), {from: vault});
    assert.equal(await balanceOf(user1), OneSdtInWei.mul(BIG(1)).toString());
    await coin.transfer(user2, OneSdtInWei.mul(BIG(1)), {from: user1});
    assert.equal(await balanceOf(user1), OneSdtInWei.mul(BIG(0)).toString());
    assert.equal(await balanceOf(user2), OneSdtInWei.mul(BIG(1)).toString());
  });
  ...
  it("reserve and reserveOf", async () => {
    ...
    await coin.transfer(user1, OneSdtInWei.mul(BIG(10)), { from: vault });
    await coin.setReserve(user1, OneSdtInWei.mul(BIG(5)), { from: admin });
    assert.equal(await reserveOf(user1), OneSdtInWei.mul(BIG(5)).toString());
  });
  
  it("transfer after setting reservation", async () => {
    // user1 = 10 SDT
    await coin.transfer(user1, OneSdtInWei.mul(BIG(10)), { from: vault });
    // user1 = 10 SDT with 5 SDT reserve
    await coin.setReserve(user1, OneSdtInWei.mul(BIG(5)), { from: admin });
    // user1 = 9 SDT with 5 SDT reserve
    await coin.transfer(user2, OneSdtInWei.mul(BIG(1)), {from: user1});        // keeping reserved amount
    assert.equal(await balanceOf(user1), OneSdtInWei.mul(BIG(9)).toString());

    // user1 = 6 SDT with 5 SDT reserve
    coin.transfer(user2, OneSdtInWei.mul(BIG(3)), {from: user1});        // keeping reserved amount
    assert.equal(await balanceOf(user1), OneSdtInWei.mul(BIG(6)));

    try {
      let tx = await coin.transfer(user2, OneSdtInWei.mul(BIG(2)), {from: user1}); // violating reserved amount
      assert.equal(await balanceOf(user1), OneSdtInWei.mul(BIG(6)));
      assert.fail();
    }
    catch(exception) {
